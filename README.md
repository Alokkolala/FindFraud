...
You should see commands for `train` and `score`. The editable install exposes `findfraud` as a package and `python -m findfraud.cli` as a CLI entrypoint.
...
4. Inspect outputs:
   - `outputs/scores.csv` contains `transaction_id`, `fraud_score`, `is_suspicious`, and `explanation`.
   - `outputs/report.html` is an overview report. If `weasyprint` is installed, `outputs/report.pdf` will also be generated.
